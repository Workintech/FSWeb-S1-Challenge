...
function indekseGoreFenomen(fenomen1, indexFenomen) {
  let a =
    indexFenomen +
    ". indekste bulunan fenomen: " +
    fenomen1[indexFenomen].profile;
  // let a = `${indexFenomen}. indekste bulunan fenomen:
  //   ${fenomen1[indexFenomen].profile}`;
  return a;
}
...
function fenomenEkle(
  ...
  let yeniObj = {};
  yeniObj.number = gelenNumber;
  yeniObj.profile = gelenProfile;
  yeniObj.followers = gelenFollowers;
  yeniObj.posts = gelenPosts;
  yeniObj.platform = gelenPlatform;
  let yeniFenomenler = [...ekle];
  yeniFenomenler.push(yeniObj);
...
*/
console.clear();
function fenomenGonderimSayisi(fenomenGönderim, fenomenAdi) {
  let fenomenPostSayısı;
  for (let i = 0; i < fenomenGönderim.length; i++) {
    if (fenomenAdi === fenomenGönderim[i].profile) {
      fenomenPostSayısı = fenomenGönderim[i].posts;
    }
  }
...
function platformaGoreCokGonderiYapanFenomen(fenomen2, platform) {
  let fenomenn;
  let gönderiMax = 0;
  for (let i = 0; i < fenomen2.length; i++) {
    console.log(i);
    if (fenomen2[i].platform === platform) {
      if (fenomen2[i].posts !== "NA" && fenomen2[i].posts > gönderiMax) {
        gönderiMax = fenomen2[i].posts;
        fenomenn = fenomen2[i].profile;
      }
    }
  }
  return fenomenn;
